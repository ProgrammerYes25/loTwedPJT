...
async function getData (dataList){
  ...
  if (dataList.exists()) {
    ...
    dataArray.forEach((record) => {
      ...
      tr.innerHTML = `
            <td>${record.name}</td>
            <td>${typeof record.temperature === 'number' ? record.temperature.toFixed(2) + '°C' : record.temperature}</td>
            <td>${typeof record.humidity === 'number' ? record.humidity.toFixed(2) + '%' : record.humidity}</td>
            <td>${typeof record.pressure === 'number' ? record.pressure.toFixed(2) + 'hPa' : record.pressure}</td>
            <td>${record.gyro_x !== 'N/A' ? `X:${record.gyro_x.toFixed(2)}, Y:${record.gyro_y.toFixed(2)}, Z:${record.gyro_z.toFixed(2)}` : 'N/A'}</td>
            <td>${record.accel_x !== 'N/A' ? `X:${record.accel_x.toFixed(3)}, Y:${record.accel_y.toFixed(3)}, Z:${record.accel_z.toFixed(3)}` : 'N/A'}</td>
